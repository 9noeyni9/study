...
class ATM
{
	...
	synchronized void deposit(int amount, String name) { //synchronized는 동시접근을 막는 키워드야 synchronized에 없이 실행 했다면 금액들이 맞지 않아
		total += amount;
...
class ATM_USER extends Thread //ATM 사용자(는 스레드를 통해서 만들 것)
{
	...
	public ATM_USER(ATM obj, String name) {
		super(name);  //String kbs / super(kbs)더라도 name이라는 변수로 저장되있고 불러오는 방식이 밑에 getName인 것
		this.obj =obj;
	}
...
public class MyATM {
	...
	public static void main(String[] args) {  
		...
		//그 통장 하나에 연결된 카드 세개를 세 사람이 나눠 가졌다고 생각하기
